...
for i in range(numBeads):
    ...
    j = i
    curr = 0
    #!The ultimate goal here is to find maximum of 2 segments because an incision can be made in between.
    #*But for "w" we can have an extra advantage. 
    #*Honestly think this is pretty much same as my first solution. 
    while state <= 2:
        
        while j < numBeads+i and (beads[j]==c or beads[j]=="w"):
            curr +=1
            j+=1
        state +=1
        #*I finally understand why c = beads[j] exists. 
        #*After the possible long sequence of "w", we need to get whatever is the first non-"w". 
        #*So that we can conitnue (though not where we left off). 
        c = beads[j]
    curr_max = max(curr, curr_max)
    
fout.write(f"{curr_max}\n")
