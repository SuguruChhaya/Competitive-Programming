...
//!The issue is that it doesn't have to be a 2 distinct regions. Could be 3 regions that would be connected 
...
int main(){
    ...
    vector<long long> allRegions;
    for (long long i=0;i<n;i++){
        for (long long j=0;j<n;j++){
            if (!visited[i][j]){
                curr = 0;
                dfs(i, j, arr[i][j], currRegion);
